...
最近一个让我非常惊讶的编程趋势，无论是在工作中还是在开源项目中，都有人突然开始这样做：

（1）撰写大量相关、早该完成、犀利、简洁、切中要点的文档……却将其放在 copilot-instructions.md 或 AGENTS.md 文件中。

（2）实现极具价值的工作流，却仅以 skills 或 MCP 服务器的形式对外提供。

（3）改进了测试和命令行工具的输出效果，却仅通过面向 AI 的标志和环境变量（如 AI=true）来启用。
